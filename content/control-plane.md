...
| Concept | Role |
|---|---|
| Agent identity blueprint | The template application object an agent identity is created from. It holds a credential of its own, which it uses to authenticate itself to Microsoft Entra ID and then mint a token on behalf of each agent identity created from it (see "Workload identity federation" below for what that credential can be). Conditional Access and permission grants applied at the blueprint level are inherited by every agent instance created from it. |
| Agent identity blueprint principal | A service principal, auto-created when the blueprint is instantiated in a tenant, representing the blueprint itself in the tenant's directory. It's what lets the blueprint obtain its own app-only Microsoft Graph tokens to create and manage agent identities — a distinct concern from the credential (managed identity or client secret) the blueprint separately uses to impersonate those agent identities toward other resources. |
| Individual agent identity | A service principal created from a blueprint, representing one specific agent instance. It can't hold its own credentials; it relies on the blueprint's impersonation flow to acquire tokens for it, but it's still what appears as the client in the issued token and in audit logs, not the blueprint. It's the principal that RBAC role assignments and per-agent policy decisions should attach to. |
...
| Workload identity federation | One way the blueprint can authenticate to Microsoft Entra ID without storing a secret: a federated identity credential that trusts another identity as its proof of identity, instead of a password or certificate. A managed identity is the typical trust source, and is recommended for an Azure-hosted production blueprint. The alternative is a plain client secret — simpler to set up, common for local development, but a stored secret that has to be rotated and protected. |
| Managed identity (hosting component) | Secures the Azure-hosted workload that runs the agent's code — a container app, a function, a VM. It is not itself the agent, and where a distinct agent identity exists, it should not be the principal downstream RBAC roles are granted to. |
...
- **A blueprint's credential compromise has blast radius across every agent identity it mints tokens for.** Agent identities don't hold their own credentials; they rely entirely on the blueprint's impersonation flow. A compromised blueprint credential is a compromise of every agent instance created from that blueprint, not just one. How many agent identities a single blueprint is allowed to back is itself a security boundary decision, not just a provisioning convenience.
